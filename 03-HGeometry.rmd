...
:::{.theorem #qgeoStab name="quasi-geodesic approximation in hyperbolic spaces (Theorem 7.2.10)"}
In a $\eta$-hyperbolic space $X$, every $(\alpha, \beta)$-quasi-geodesic $\gamma:I\to X$ is 
contained in the $\delta$-neighborhood of every geodesic $[\gamma(0), \gamma(b)]$, where 
$\delta$ only depends upon $\eta, \alpha, \beta$. 
:::

:::{.lemma #christmas name="christmas tree lemma"}
In an $\eta$-hyperbolic space $X$, to every continuous curve $\gamma:[0, L]\to X$ and 
geodesic $[\gamma(0), \gamma(L)]=\gamma':[0, L']\to X$, the distance from $[\gamma(0), \gamma(L)]$ to 
$\mrm{im}(\gamma)$ is bounded by the log-length of $\gamma$: 
\[ 
    \sup_{t\in [0, L']} d(\gamma'_t, \mrm{im}\, \gamma) \leq \delta \cdot | \log_2 \circ L_X(\gamma) | + 1 
\] 
:::
...
This section models section 7.4.1, which efficiently solves the word problem for hyperbolic groups. 
...
:::{.lemma #hShortcuts name="shortcuts in cycles in hyperbolic groups"}
Given a hyperbolic group $G$, a finite generating set $S$ and $\Delta(G, S)$ being $\eta$-hyperbolic, 
